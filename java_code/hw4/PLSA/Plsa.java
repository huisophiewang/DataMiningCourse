...
public class Plsa {
    ...
    private void em() {
        ...
        for (int docIndex = 0; docIndex < docSize; docIndex++) {
            for (int wordIndex = 0; wordIndex < vocabularySize; wordIndex++) {
                ...
                for (int topicIndex = 0; topicIndex < topicNum; topicIndex++) {	
                    ...
                    perTopicPro[topicIndex] = docTopicPros[docIndex][topicIndex] * topicTermPros[topicIndex][wordIndex];
                    total += perTopicPro[topicIndex];
                }
        ...
        for (int topicIndex = 0; topicIndex < topicNum; topicIndex++) {
        	...
        	Double[] termProsDouble = new Double[vocabularySize];
        	for (int i = 0; i < vocabularySize; i++){
        		termProsDouble[i] = topicTermPros[topicIndex][i];
        	}
        	
        	ArrayIndexComparator comparator = new ArrayIndexComparator(termProsDouble);
        	Integer[] indexes = comparator.createIndexArray();      	
        	Arrays.sort(indexes, comparator);
        	
        	for (int k=0; k<10; k++)     	
        		System.out.println(allWords.get(indexes[k]));
        ...
        // print out doc topic prob (and sort topic index in descending order)
        for (int docIndex = 0; docIndex < docSize; docIndex++) {
        	//System.out.println(docIndex+1 + " " + Arrays.toString(docTopicPros[docIndex]));
        	
        	Double[] topicProsDouble = new Double[topicNum];
        	for (int i = 0; i < topicNum; i++)
        		topicProsDouble[i] = docTopicPros[docIndex][i];
            ArrayIndexComparator comparator = new ArrayIndexComparator(topicProsDouble);
        	Integer[] indexes = comparator.createIndexArray();      	
        	Arrays.sort(indexes, comparator);
        	System.out.println(docIndex+1 + " " + Arrays.toString(indexes));
        }
